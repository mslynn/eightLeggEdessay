...
// 读取主文件
const mainFile = path.join(__dirname, '低代码面试题集锦.md');
const content = fs.readFileSync(mainFile, 'utf-8');

// 移除开头的标题行（如果存在）
let processedContent = content;
const titleIndex = processedContent.indexOf('\n## ');
if (titleIndex !== -1) {
  processedContent = processedContent.substring(titleIndex + 1);
}
...
// 创建 questions 目录
const questionsDir = path.join(__dirname, 'questions-lowcode');
...
// 生成索引文件
const indexContent = `# 低代码平台面试题集锦（截止 2025 年底）
...
${questions.map((question, index) => {
  ...
  const safeTitle = title.replace(/[\/\\:*?"<>|？。,、（）\(\)]/g, '_').replace(/\s+/g, '_').replace(/[\.]/g, '_');
  const fileName = `${String(questionNumber).padStart(2, '0')}__${questionNumber}__${safeTitle}.md`;
  return `${questionNumber}. [${title}](./questions-lowcode/${fileName})`;
}).join('\n')}
...
[查看详细答案](./questions-lowcode/${fileName})`;
}).join('\n')}
`;
