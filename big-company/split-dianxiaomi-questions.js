...
const mainFile = path.join(__dirname, '店小秘面试题集锦.md');
const outputDir = path.join(__dirname, 'questions-dianxiaomi');
const indexFile = path.join(__dirname, 'index-dianxiaomi.md');
...
// 生成索引文件
const indexContent = `# 店小秘面试题集锦

本目录包含店小秘前端面试题，按题目编号组织。

## 题目列表

${questions.map(q => {
  const safeTitle = q.title.replace(/[\/\\:*?"<>|?。，、（）\(\)]/g, '_').replace(/\s+/g, '_');
  return `${q.number}. [${q.number}. ${q.title}](./questions-dianxiaomi/${String(q.number).padStart(2, '0')}__${q.number}__${safeTitle}.md)`;
}).join('\n')}
