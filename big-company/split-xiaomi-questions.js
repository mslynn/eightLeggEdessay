...
const mainFile = path.join(__dirname, '小米面试题集锦.md');
const outputDir = path.join(__dirname, 'questions-xiaomi');
const indexFile = path.join(__dirname, 'index-xiaomi.md');
...
// 生成索引文件
const indexContent = `# 小米面试题集锦

本目录包含小米前端面试题，按题目编号组织。

## 题目列表

${questions.map(q => {
  const safeTitle = q.title.replace(/[\/\\:*?"<>|？。,、（）\(\)\[\]]/g, '_').replace(/\s+/g, '_');
  return `${q.number}. [${q.number}. ${q.title}](./questions-xiaomi/${String(q.number).padStart(2, '0')}__${q.number}__${safeTitle}.md)`;
}).join('\n')}
